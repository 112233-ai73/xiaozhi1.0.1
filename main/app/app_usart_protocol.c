...
static void process_normal_command(uint8_t cmd_id, const uint8_t *data, uint8_t data_len) {
    (void)data;

    // 使用数学规律简化 0x6A ~ 0x76 的 MP3 异步播放指令
    if (cmd_id >= 0x6A && cmd_id <= 0x76 && (cmd_id % 2 == 0)) {
        int file_num = 200 + (cmd_id - 0x6A) / 2;
        char filename[16];
        snprintf(filename, sizeof(filename), "%d.mp3", file_num);
        audio_mp3_play_file_async(filename);
        return;
    }
...
static void dispatch_normal_packet(uint8_t cmd_id, const uint8_t *packet, uint8_t packet_len) {
    ...
    process_normal_command(cmd_id, data_payload, data_len);
}
...
void app_usart_protocol_handle_packet(const uint8_t *packet, uint8_t packet_len) {
    ...
    if (is_wifi_fragment_command(cmd_id)) {
        dispatch_wifi_fragment_packet(cmd_id, packet, packet_len);
    } else {
        dispatch_normal_packet(cmd_id, packet, packet_len);
    }
}